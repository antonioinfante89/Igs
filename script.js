document.addEventListener('DOMContentLoaded', function() {
    ...
    projectForm.addEventListener('submit', function(e) {
        ...
        const resourceId = parseInt(this.querySelector('select[name="projectResource"]').value);
        const time = parseFloat(this.querySelector('[name="projectTime"]').value);
        
        const markups = {
            markup1: this.querySelector('input[name="markup1"]').checked,
            markup2: this.querySelector('input[name="markup2"]').checked,
            markup3: this.querySelector('input[name="markup3"]').checked,
            markup4: this.querySelector('input[name="markup4"]').checked,
            markup5: this.querySelector('input[name="markup5"]').checked
        };

        console.log('Project data:', { projectId, name, resourceId, time, markups });
    ...
    function resetProjectForm() {
        ...
        const markup1Checkbox = projectForm.querySelector('input[name="markup1"]');
        if (markup1Checkbox) markup1Checkbox.checked = true;
    }
    ...
    function editProject(id) {
        ...
        if (project) {
            ...
            projectForm.querySelector('select[name="projectResource"]').value = project.resourceId;
            projectForm.querySelector('[name="projectTime"]').value = project.time;
            
            const checkboxes = ['markup1', 'markup2', 'markup3', 'markup4', 'markup5'];
            checkboxes.forEach(markup => {
                const checkbox = projectForm.querySelector(`input[name="${markup}"]`);
                if (checkbox) checkbox.checked = project.markups[markup];
            });
    ...
    function updateUI() {
        ...
        resourceTableBody.innerHTML = resources.map(resource => `
            <tr>
                ...
                <td>
                    <button onclick="window.editResource(${resource.id})" class="btn btn-primary">Modifica</button>
                    <button onclick="window.deleteResource(${resource.id})" class="btn btn-danger">Elimina</button>
                </td>
            </tr>
        `).join('');
        ...
        projectTableBody.innerHTML = projects.map(project => {
            ...
            return `
                <tr>
                    ...
                    <td>
                        <button onclick="window.editProject(${project.id})" class="btn btn-primary">Modifica</button>
                        <button onclick="window.deleteProject(${project.id})" class="btn btn-danger">Elimina</button>
                    </td>
                </tr>
            `;
        }).join('');

        const projectResourceSelect = projectForm.querySelector('select[name="projectResource"]');
        if (projectResourceSelect) {
